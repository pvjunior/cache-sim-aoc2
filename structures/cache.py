...
class Cache:
    ...
    def read(self, address: int) -> int:
        ...
        if found:
            ...
            if block_address not in self._accessed_blocks:
                ...
                set_blocks = self._fetch_set(index)
                valid_count = sum(1 for b in set_blocks if b.valid != 0)
                if valid_count == self.associativity:
                    self.stats["conflict_miss"] += 1
                else:
    ...
    def write(self, address: int, data: int):
        ...
        # If it was a miss, update the block address tracking and classify miss
        if not found:
            block_address = address & (~((1 << self._offset) - 1))
            if block_address not in self._accessed_blocks:
                self._accessed_blocks.append(block_address)
    ...
    def _insert_byte_in_position(
        self, current_block_payload: int, data_to_insert: int, offset: int
    ) -> int:
    ...
    def pick_block_by_politic(self, set: tuple) -> DataBlock:
        ...
        match self.replacement:
            ...
            case "FIFO":
                ...
                self.FIFO_tracking[index] = (
                    self.FIFO_tracking[index] + 1
                ) % self.associativity
    ...
    def replacement(self, val):
        ...
        match val:
            ...
            case _:
                raise ValueError(
                    f"{val} not an option for replacement strategy. Available options: {replOPTS}"
                )
    ...
    def addressing(self, addressing):
        ...
        if not addressing in addrOPTS:
            raise ValueError(
                f"{addressing} bit addresses not supported, only: {addrOPTS}"
            )
...
class _PLRU:
    def __init__(self, nsets: int, associativity: int):
        if associativity == 1:
            raise BaseException(
                "Associativity is 1. Something went very wrong in there pal."
            )
        self.sets = []
